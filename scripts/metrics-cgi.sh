...
SELECT
  ...
  '# HELP route10_pon_omci_log_bytes APPARENT size of the stick OMCI log before this cycle pull - it equals every byte omci_app has written since its write offset was last reset, NOT resident data (the file is ~99 percent sparse hole). The 2026-08-18 collapse curve: reading it materialises that many bytes of unreclaimable ramfs on a 23 MB-total box, so pon-collect refuses to pull above 4 MB (2.5x under the ~10 MB measured kill point, wide enough for the 1.7 MB per 150 s incident firehose). Alert if this trends toward the cap - that is days of warning, and the 0-byte baseline right after a good drain is normal.' || char(10) ||
  'route10_pon_omci_log_bytes '         || COALESCE(json_extract(json,'\$.omcilog.bytes'),'NaN')  || char(10) ||
  '# HELP route10_pon_stick_mem_free_bytes MemFree on the ODI stick (23 MB total RAM, ~2.2 MB free healthy fresh-boot). The management-plane OOM early warning the 2026-08-18 postmortem demanded: userspace collapsed top-down when a whole-file log read exhausted this. Watch the trend, not a threshold - the healthy value is already small.' || char(10) ||
  'route10_pon_stick_mem_free_bytes '   || COALESCE(json_extract(json,'\$.mem_free_kb')*1024,'NaN') || char(10) ||
